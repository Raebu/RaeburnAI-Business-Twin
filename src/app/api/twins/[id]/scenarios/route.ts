...
const Input = z.object({
  ...
  scenario: z.union([
    z.object({
      type: z.literal('capacity_loss'),
      role: z.string().min(1),
      peopleLost: z.number().int().positive().max(1000),
    }),
    z.object({
      type: z.literal('workflow_volume_change'),
      workflowId: z.string().min(1),
      percentageChange: z.number().min(-100).max(1000),
    }),
    z.object({
